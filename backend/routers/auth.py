...
async def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    pool = await get_pool()
    user = await pool.fetchrow("SELECT id, email, role, created_at FROM users WHERE email = $1", token_data.email)
...
async def register(user_in: UserCreate):
    ...
    new_user = await pool.fetchrow(
        "INSERT INTO users (email, role, hashed_password) VALUES ($1, $2, $3) RETURNING id, email, role, created_at",
        user_in.email, user_in.role.value, hashed_password
    )
