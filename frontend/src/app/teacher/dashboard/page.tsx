...
export default function TeacherDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setStudents([
            { id: 'DS', name: 'Dylan S.', lastActive: '2m ago', risk: 'Critical', modules: '4/12', dotColor: 'bg-red-400 animate-pulse', riskColor: 'text-red-400' },
          ...
          {students.map((student, idx) => (
            <motion.div key={student.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ ...springTransition, delay: idx * 0.1 }} className="bg-surface-container-high rounded-2xl p-6 border border-outline-variant/15 hover:border-primary/30 transition-all group relative overflow-hidden">
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="h-12 w-12 rounded-full bg-primary/20 flex items-center justify-center font-headline font-bold text-lg text-primary">{student.id}</div>
                <div>
