        ...
        {open && (
          ...
          >
            ...
            {DEMO_ACCOUNTS.map((a) => (
              ...
              >
                <div className={`p-1 rounded-md ${a.role === 'teacher' ? 'bg-primary/10 text-primary' : a.role === 'parent' ? 'bg-orange-400/10 text-orange-400' : a.role === 'admin' ? 'bg-purple-400/10 text-purple-400' : 'bg-secondary/10 text-secondary'}`}>
                  {a.role === 'teacher' ? <Presentation size={13} /> : a.role === 'parent' ? <Heart size={13} /> : a.role === 'admin' ? <Shield size={13} /> : <GraduationCap size={13} />}
                </div>
